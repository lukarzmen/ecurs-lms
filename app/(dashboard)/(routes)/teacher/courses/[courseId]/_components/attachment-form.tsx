        ...
        <div className="mt-6 border bg-slate-100 rounded-md p-4">
            <div className="font-medium flex items-center justify-between">
                ...
                <Button onClick={toogleEdit} variant="ghost">
                    {isEditing && (<>Cancel</>) }
                    {!isEditing &&  (<><PlusCircle className="h-4 w-4 mr-2"></PlusCircle>
                        Add file</>)
                        }
            ...
            {!isEditing && !blobUrlsEmpty ? (
                blobUrls.map((blobUrl) => (
                    <div className="flex items-center p-3 w-full bg-sky-100 border-sky-200 border text-sky-700 rounded-md">
                        ...
                        <button type="button" className="ml-auto hover:opacity-75 transition" onClick={() => onDelete(blobUrl)} title="Delete Attachment">
                            <X className="h-4 w-4">
                            </X>
                        </button>
                    </div>
