...
export async function PATCH(
  ...
  try {
    ...
    const parsedEducationalPathId = parseInt(id, 10);

    // Basic validation
    const priceNumber = Number(rawPrice);
    const allowedIntervals = ["ONE_TIME", "MONTH", "YEAR"];

    if (isNaN(parsedEducationalPathId) || isNaN(priceNumber) || priceNumber < 0) {
    ...
    const createData: any = {
      ...
      isRecurring,
      interval,
      educationalPathId: parsedEducationalPathId,
      trialPeriodType,
    ...
    const upserted = await prisma.educationalPathPrice.upsert({
      where: { educationalPathId: parsedEducationalPathId },
      create: createData,
      update: updateData,
    });
